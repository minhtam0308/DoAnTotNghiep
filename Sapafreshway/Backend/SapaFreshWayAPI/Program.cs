...
using SapaFreshWayAPI.Services;
using System.Text;
...
ExcelPackage.License.SetNonCommercialOrganization("SapaFreshWay");

var builder = WebApplication.CreateBuilder(args);

builder.Services.AddDbContext<SapaFreshContext>(options =>
{
...
builder.Services.AddSingleton<SapaFreshWayAPI.Services.CloudinaryService>();
...
builder.Services.AddAuthorization(options =>
{
    ...
    options.AddPolicy(Roles.Manager, p => p.RequireRole(Roles.Manager, Roles.Admin, Roles.Owner));
    options.AddPolicy(Roles.Staff, p => p.RequireRole(Roles.Staff, Roles.Admin, Roles.Manager, Roles.Owner));
    options.AddPolicy(Roles.Customer, p => p.RequireRole(Roles.Customer));
    options.AddPolicy(Roles.Owner, p => p.RequireRole(Roles.Owner, Roles.Admin, Roles.Manager));
...
builder.Services.AddCors(options =>
{
    options.AddPolicy(name: MyAllowSpecificOrigins, policy =>
    {
        ...
        var allowedOrigins = builder.Configuration.GetSection("CorsSettings:AllowedOrigins").Get<string[]>() 
            ?? new[] { "http://localhost:5123", "http://192.168.79.29:5123"
...
{
    var ctx = scope.ServiceProvider.GetRequiredService<SapaFreshContext>();
