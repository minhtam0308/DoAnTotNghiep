...
namespace SapaFreshWayForStaff.DTOs.Payment
{
    ...
    public class ReservationPaymentDto
    {
        ...
        // Legacy aliases for compatibility with existing Payment.cshtml
        [JsonIgnore]
        public decimal Total => TotalAmount;

        [JsonIgnore]
        public int OrderId => Orders?.FirstOrDefault()?.OrderId ?? 0;

        [JsonIgnore]
        public string? OrderCode => Orders?.FirstOrDefault()?.OrderCode;
